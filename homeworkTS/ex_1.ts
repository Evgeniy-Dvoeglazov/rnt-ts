...
class DoubleLinkedListNode<T> {
  constructor(
    ...
    public previous: DoubleLinkedListNode<T> | null = null,
  ) {}
}
...
export default class DoubleLinkedList<T> {
  ...
  find(value: T): DoubleLinkedListNode<T> | null {
    if (!this.first) {
      return null;
    }
  ...
  pop(): DoubleLinkedListNode<T> | null {
    if (!this.last) {
      return null;
    }
  ...
  shift(): DoubleLinkedListNode<T> | null {
    if (!this.first) {
      return null;
    }
  ...
  toArray(): T[] {
    const nodeValues = [];
    for (const node of this) {
      node && nodeValues.push(node.value);
    }
  ...
  reverse(): DoubleLinkedList<T> {
    ...
    previousNode && (this.first = previousNode);
  ...
  [Symbol.iterator](): {
    next: () =>
      | { value: DoubleLinkedListNode<T>; done: boolean }
      | { done: boolean; value?: null };
  } {
